...
From 30 December 2026 the EU Deforestation Regulation requires operators to prove that products linked to seven regulated commodities are deforestation-free, evidenced by Due Diligence Statements (DDS). This project identifies which of Stonehouse's 81 own-brand products fall in scope and builds an analytics system to generate, store, validate, analyse, forecast and report on DDS. Since no real statements exist yet, the system is demonstrated on synthetic data calibrated to real supplier and volume records.
...
All synthetic data is generated under fixed random seed 42; every figure in the report regenerates identically. Run the `src/` scripts in pipeline order.
...
Sponsor-provided datasets are commercial in confidence. All DDS records in this repository are synthetic; no real supplier or consignment data is processed at any stage.
